...
def test_number_of_comments_for_posts_shown_on_list(client, test_user, single_post):
    ...
    single_post.add_ccomment("you're fucking brilliant!", test_user)
    response = client.get(url_for("index"))
    assert b"3 Comments" in response.data
...
# here is activity logging stuff to turn off for now...
# he said we could delete... but gonna just comment out for now...
...
#    ), follow_redirects=True)
#    assert response.status_code == 200
#    e = ActivityLog.latest_entry()		<-- this
