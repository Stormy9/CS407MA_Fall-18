...
@app.route("/logout")
def logout():
    ActivityLog.log_event(current_user, f"Logout {current_user}")	# removed .id
    logout_user()
    return redirect(url_for("index"))
...
def down_vote(id):
    ...
    if current_user.is_authenticated:
        ...
        ActivityLog.log_event(current_user, f"Down Vote: {post}")	# removed .id
        return redirect(next or url_for("index"))
    else:
        return redirect(url_for("login"))
...
def up_vote_comment(id):
    ...
    if current_user.is_authenticated:
        ...
        ActivityLog.log_event(current_user, f"Up Vote: {comment}")	# removed .id
        return redirect(next or url_for("index"))
    else:
        return redirect(url_for("login"))


# part of activity logging...update ActivityLog.log_event()   (9)
...
def down_vote_comment(id):
    ...
    if current_user.is_authenticated:
        ...
        ActivityLog.log_event(current_user, f"Down Vote: {comment}")	# removed .id
        return redirect(next or url_for("index"))
    else:
        return redirect(url_for("login"))
